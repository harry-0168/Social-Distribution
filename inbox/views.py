...
def inboxApi(request, object_author_serial):
    ...
    try:
        ...
        if parsed_data['type'] == 'follow':
            ...
            comment = Comment(author=['author']['id'], username=parsed_data['author']['username'], comment=parsed_data['comment'], published=parsed_data['published'], id=parsed_data['id'] , uuid=parsed_data['uuid'], post=post)
            print("\nComment: ", comment)
            comment.save()
            print("comment saved")
