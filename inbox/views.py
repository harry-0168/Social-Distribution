...
def inboxApi(request, object_author_serial):
    ...
    try:
        ...
        if parsed_data['type'] == 'follow':
            ...
            comment = Comment(author=object_author, username= parsed_data['object']['username'], comment= parsed_data['object']['comment'], id=parsed_data['object']['id'] , uuid=parsed_data['object']['uuid'], post=post)
            print("\nComment: ", comment)
            comment.save()
            print("comment saved")
